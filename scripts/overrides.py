...
OVERRIDES = [
	(
		"A record OneAI made carries the OneAI mark in every list",
		"intake.js wraps ListView.get_subject_element to put the mark before the title, and render to ask once per page which rows carry it; the long list draws rows as it scrolls, so the title is the one place every row passes",
		"onedesk/public/js/intake.js",
		"frappe/frappe/public/js/frappe/list/list_view.js",
		"get_subject_element(doc, title) {",
	),
	(
		"OneAI tags a file with Frappe's own tags",
		"DocTags.add checks write permission for whoever is signed in and the OneAI user holds none, so act.py makes the same two writes, the _user_tags column and a Tag Link, after checking the person OneAI acts for",
		"onedesk/one_intake/act.py",
		"frappe/frappe/desk/doctype/tag/tag.py",
		"def update_tags(doc, tags):",
	),
	(
		"A record's Files tab lists the files linked to it as well as those attached",
		"a File has one attached_to; a document that belongs to several records is attached to the main one and File-Linked to the rest, and namespace.attachments adds those the reader may open",
		"onedesk/one_storage/namespace.py",
		"frappe/frappe/core/doctype/file/file.py",
		"def create_attachment_copy(",
	),
	(
		"The email composer signs as the sending address, not the writer",
		"mail_compose.js wraps CommunicationComposer.get_signature, which prefers the writer's own User signature, so a shared address signs the same whoever writes",
	...
	(
		...
		"frappe/frappe/core/doctype/communication/communication.py",
		"if not self.flags.skip_add_signature:",
	),
	(
		"A form's timeline lists only the mail its reader may open",
		"Frappe lists every message linked to a record to whoever reads the record; a link must not grant read",
		"onedesk/one_mail/linking.py",
		"frappe/frappe/desk/form/load.py",
		'frappe.response["docinfo"] = docinfo',
	),
	(
		"A message opens only for its mailbox's holders",
		"Frappe narrows lists of Communication by User Email but opens any one by name to an Inbox User",
		"onedesk/one_mail/access.py",
		"frappe/frappe/core/doctype/communication/communication.py",
	...
	(
		"An approval asked and answered is told through One's hub",
		"HRMS tells both moments only to its phone app, as PWA Notification rows; tell.Approvals overrides the mixin's two methods on Leave Application, Expense Claim and Shift Request",
		"onedesk/one_hr/tell.py",
		"hrms/hrms/mixins/pwa_notifications.py",
		"def notify_approval_status(self):",
	),
	(
		"Leave's own mails are silent",
		"HRMS mailed the approver and the employee from Email Templates; the approval types tell them, so tell.LeaveApplication makes both methods do nothing and the switch is off",
		"onedesk/one_hr/tell.py",
		"hrms/hrms/hr/doctype/leave_application/leave_application.py",
		"def notify_leave_approver(self):",
	),
	(
		"A moved interview tells interviewers in One and mails the applicant",
		"HRMS mails everybody in fixed words, with no switch; tell.Interview writes reschedule_interview again",
		"onedesk/one_hr/tell.py",
		"hrms/hrms/hr/doctype/interview/interview.py",
		"def reschedule_interview(",
	),
	(
		"HRMS's reminders are told by our jobs",
		"birthdays, anniversaries, holidays and the two interview reminders are mailed by HRMS when its HR Settings switch is on; notify._settle keeps each switch off and tell.py sends them",
		"onedesk/one_hr/tell.py",
		"hrms/hrms/controllers/employee_reminders.py",
		'to_send = int(frappe.db.get_single_value("HR Settings", "send_birthday_reminders"))',
	),
	(
		"The interview reminder waits on HRMS's own switch being off",
		"send_interview_reminder is checked before it mails; with it on, the interviewers would hear twice",
		"onedesk/one_hr/tell.py",
		"hrms/hrms/hr/doctype/interview/interview.py",
		"if not cint(reminder_settings.send_interview_reminder):",
	),
	(
		"The credit limit dialog tells the credit controllers in One",
		"its primary action calls customer.send_emails by name, which one_book/tell.credit_limit stands in for",
		"onedesk/one_book/tell.py",
		"erpnext/erpnext/selling/doctype/customer/customer.py",
		'"server_action": "erpnext.selling.doctype.customer.customer.send_emails",',
	),
	(
		"A request raised by reordering is known by its rows",
		"reorder_item writes each row's reorder level, which one_inventory/tell.raised reads to tell Purchasing; Stock Settings' mail switch is off",
		"onedesk/one_inventory/tell.py",
		"erpnext/erpnext/stock/reorder_item.py",
		'"reorder_level": d.reorder_level,',
	),
	(
		"A standard rule's recipients here are told through the hub",
		"Rule.send_an_email tells the people in the workspace and hands frappe's send_an_email only the addresses that are nobody here, through get_list_of_recipients",
		"onedesk/one/rules.py",
		"frappe/frappe/email/doctype/notification/notification.py",
	...
	(
		"A primary verb is the one dark button on a record",
		"ERPNext makes the Create group primary after our refresh; the form's `set_inner_btn_group_as_primary` skips it while a primary verb, Record Payment on an invoice or bill, is shown",
	...
	(
		"A record's head wraps the page's primary-group setter",
		"one dark button on a record; the wrap is per form",
		"onedesk/public/js/head.js",
